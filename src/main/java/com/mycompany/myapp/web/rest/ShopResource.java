...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Shop}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Shop> partialUpdateShop(@PathVariable(value = "id", required = false) final Long id, @RequestBody Shop shop)
        ...
        Optional<Shop> result = shopRepository
            ...
            .map(existingShop -> {
                ...
                if (shop.getAverageDeliveryTime() != null) {
                    existingShop.setAverageDeliveryTime(shop.getAverageDeliveryTime());
                }
                if (shop.getClosingHour() != null) {
                    existingShop.setClosingHour(shop.getClosingHour());
                }
                if (shop.getOpeningHour() != null) {
                    existingShop.setOpeningHour(shop.getOpeningHour());
                }
                if (shop.getTags() != null) {
                    existingShop.setTags(shop.getTags());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, shop.getId().toString())
        );
    }
